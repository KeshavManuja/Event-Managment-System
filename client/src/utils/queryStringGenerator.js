export const queryPath = (payload) => {
    const pathMaker = Object.keys(payload).map(function (key) {
        ...
        if (key === "page") return `_page=${payload[key]}`
        else if (key === 'rating' && payload[key]) return `rating_gte=${payload[key]}`
        else if(key === 'startDate' || key === 'endDate' ) return `${key}=${payload[key]}`
